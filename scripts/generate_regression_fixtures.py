...
PROVIDER_TEMPLATES = {
    ...
    "factura_a": {
        ...
        "extra_fields": {
            ...
            "iva_21": lambda: format_amount(generate_amount() * 0.21),
            "iva_10_5": lambda: format_amount(generate_amount() * 0.105),
...
def format_amount(value: float) -> str:
    """Formatea monto en formato AR: $ 33.867,30"""
...
def render_template(provider: str, doc_type: str, variant: str, dpi: int):
    ...
    # Header
    y = int(50 * dpi / 300)
    x_start = int(50 * dpi / 300)
    font_title = get_font(int(24 * dpi / 300), bold=True)
    draw.text((x_start, y), template.get("display_name", provider.upper()), fill="black", font=font_title)
    y += int(35 * dpi / 300)
    font_normal = get_font(int(16 * dpi / 300))
    draw.text((x_start, y), template.get("direccion", ""), fill="black", font=font_normal)
    y += int(35 * dpi / 300)
    cuit_text = f"CUIT: {template.get('cuit', '')}"
    draw.text((x_start, y), cuit_text, fill="black", font=font_normal)
    y += int(50 * dpi / 300)
    ...
    x_start = int(50 * dpi / 300)
    x_end = width - x_start
    draw.line([(x_start, y), (x_end, y)], fill="gray", width=1)
    y += int(20 * dpi / 300)
    footer_text = "Documento generado automáticamente para testing de regresión OCR"
    font_small = get_font(int(12 * dpi / 300))
    draw.text((x_start, y), footer_text, fill="gray", font=font_small)
...
def generate_fixtures(
    provider: str, count: int, output_dir: Path, templates_dir: Path, expected_dir: Path, metadata: Dict[str, Any]
) -> List[Dict[str, Any]]:
    ...
    for i in range(count):
        ...
        for field in fields:
            if field in field_data:
                ...
                expected["fields"][field] = {"value": value, "confidence": round(random.uniform(0.85, 0.99), 2)}

        expected["validation"] = {"semantic_checks_passed": True, "warnings": []}
...
def main():
    ...
    provider_choices = list(PROVIDER_TEMPLATES.keys()) + ["all", "configured"]
    parser.add_argument(
        "--provider",
    ...
    parser.add_argument("--count", type=int, default=5, help="Cantidad de fixtures por proveedor (default: 5)")
    parser.add_argument(
        "--fixtures-dir",
        type=Path,
    ...
    parser.add_argument("--version", type=str, default=None, help="Versión de fixtures (semver)")
    ...
    for provider in providers_to_generate:
        ...
        generated = generate_fixtures(provider, args.count, images_dir, templates_dir, expected_dir, metadata)
