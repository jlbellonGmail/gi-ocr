...
from backend.tests.regression_helpers import extract_text, extract_fields, validate_semantic, run_full_pipeline
...
def compare_fields(validated: Dict[str, Any], expected_fields: Dict[str, Any]) -> ComparisonResult:
    ...
    return ComparisonResult(
        ...
        by_field=comparisons,
        passed=matched_fields == len(all_fields)
    )
...
def test_regression_field_accuracy(img_meta, expected_path, image_path, regression_config):
    ...
    assert expected.get("fixtures_version") == metadata.get("fixtures_version"), \
        f"Versión de fixtures mismatch: expected {expected.get('fixtures_version')}, metadata {metadata.get('fixtures_version')}"
